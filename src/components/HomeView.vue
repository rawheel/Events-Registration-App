<template>
  ...
  >
    <div v-if="dataevent.length">
      <div
        class="flex mx-auto bg-gray-900 rounded-lg w-4/5 p-3 md:p-5 mb-2"
        v-for="(event, index) in dataevent"
        :key="index"
      >
        <div class="mr-2">
        ...
        <div class="flex-1" @click="checkdiv($event, event.id)">
          ...
          <div class="text-xs text-gray-400">
            ...
            />
            {{ event.place }}
            <span v-if="event.mode === 'on'" class="text-xs text-gray-600"
              >(Online Event)</span
            >
            <span v-else class="text-xs text-gray-600">(Physical Event)</span>
          </div>
          <div class="text-xs text-gray-400">
            <font-awesome-icon
              icon="globe-asia"
...
export default {
  ...
  data() {
    return {
      imgTemp:
        "https://res.cloudinary.com/startup-grind/image/upload/c_fill,dpr_3,f_auto,g_center,h_175,q_auto:good,w_175/v1/gcs/platform-data-dsc/events/Asset%204.jpg",
      dataevent: [],
    };
  },
  methods: {
    eventDelete(event, id, index) {
      this.$api.events
        .eventDel(id)
        .then(
          (response) => console.log(response, index, "del successfull"),
